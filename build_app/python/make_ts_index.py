...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        cfts_record['resolver'] = f"https://aad.acdh.oeaw.ac.at/{record['id']}"
        record['rec_id'] = os.path.split(x)[-1]
        cfts_record['rec_id'] = record['rec_id']
        if "aad" in x:
            record['edition'] = ["Auden in Austria Digital"]
        else:
            record['edition'] = ["Auden Musulin Papers"]
...
make_index = client.collections['aad'].documents.import_(records)
print(make_index)
print('done with indexing aad')

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {'action': 'upsert'})
print(make_index)
print('done with cfts-index aad')
